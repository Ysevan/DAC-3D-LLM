...
class DAC3DClient:
    ...
    def get_inspection_history_summary(self, *, months: int = 3) -> dict[str, Any]:
        """Return a trend summary for recent DAC-3D inspection history.

        The live DAC-3D UI may publish ``result_history`` in the runtime status file.
        If that history is not available, use deterministic demo history so the
        local workstation can still demonstrate defect trend analysis.
        """
        months = max(1, int(months or 3))
        cutoff = date.today() - timedelta(days=months * 31)
        source = "status_file"
        records = self._read_result_history_from_status_file()
        if not records:
            records = deepcopy(self._mock_result_history)
            source = "demo_inspection_history"

        filtered = [
            record
            for record in records
            if self._record_date(record) is None or self._record_date(record) >= cutoff
        ]
        if not filtered:
            filtered = records
        ...
        failed = [
            ...
            if item.get("quality") is False or str(item.get("quality_label") or "") == "不合格"
        ]
        passed = sample_count - len(failed)
        defect_total = sum(int(item.get("defects_num") or 0) for item in filtered)
        defect_counter: Counter[str] = Counter()
        batch_counter: Counter[str] = Counter()
        batch_defect_counter: Counter[str] = Counter()
        monthly_counter: dict[str, Counter[str]] = defaultdict(Counter)

        for item in filtered:
            batch_id = str(item.get("batch_id") or item.get("tray_id") or "unknown")
            batch_counter[batch_id] += 1
            if item in failed:
                batch_defect_counter[batch_id] += 1
            month_key = str(item.get("detected_at") or item.get("timestamp") or "unknown")[:7]
            for defect in item.get("defects") or []:
                if not isinstance(defect, dict):
                    continue
                defect_type = str(defect.get("defect_type") or defect.get("category") or "unknown")
                defect_counter[defect_type] += 1
                monthly_counter[month_key][defect_type] += 1

        high_risk_batches = []
        for batch_id, total in batch_counter.items():
            defective = batch_defect_counter.get(batch_id, 0)
            if total and defective / total >= 0.35:
                high_risk_batches.append(
                    {
                        "batch_id": batch_id,
                        "sample_count": total,
                        "defective_count": defective,
                        "defective_rate": round(defective / total, 3),
                    }
                )
        high_risk_batches.sort(key=lambda item: item["defective_rate"], reverse=True)

        top_defects = [
            {"defect_type": name, "count": count}
            for name, count in defect_counter.most_common(5)
        ]
        month_breakdown = {
            month: dict(counter.most_common())
            for month, counter in sorted(monthly_counter.items())
        }
        top_defect = top_defects[0]["defect_type"] if top_defects else "none"
        trend_summary = (
            f"最近{months}个月共读取{sample_count}个样品，"
            f"不合格{len(failed)}个，累计缺陷{defect_total}个；"
            f"最高频缺陷为{top_defect}。"
        )
        if high_risk_batches:
            batch = high_risk_batches[0]
            trend_summary += (
                f" 批次{batch['batch_id']}的不合格率最高，"
                f"达到{batch['defective_rate']:.0%}。"
            )
        ...
        return {
            ...
            "qualified_count": passed,
            "defective_sample_count": len(failed),
            "defect_total": defect_total,
            "defect_type_counts": dict(defect_counter.most_common()),
            "top_defects": top_defects,
            "batch_counts": dict(batch_counter.most_common()),
            "high_risk_batches": high_risk_batches,
            "month_breakdown": month_breakdown,
            "recent_samples": filtered[-6:],
    ...
    def _read_result_history_from_status_file(self) -> list[dict[str, Any]]:
        """Read only historical sample records from the DAC-3D status file."""
        status = self._read_status_file()
        if not isinstance(status, dict):
            return []
        history = status.get("result_history")
    ...
    def _record_date(record: dict[str, Any]) -> date | None:
        raw_value = record.get("detected_at") or record.get("timestamp") or record.get("date")
        if not raw_value:
            return None
        try:
    ...
    @staticmethod
    def _build_mock_result_history() -> list[dict[str, Any]]:
        """Build deterministic three-month demo inspection history."""
        today = date.today()
        patterns = [
            (88, "B2026-03-A", True, []),
            (84, "B2026-03-A", False, ["scratch"]),
            (78, "B2026-03-B", True, []),
            (72, "B2026-03-B", False, ["pit"]),
            (66, "B2026-03-B", True, []),
            (60, "B2026-04-A", False, ["scratch", "edge_chip"]),
            (55, "B2026-04-A", True, []),
            (50, "B2026-04-A", False, ["scratch"]),
            (45, "B2026-04-B", True, []),
            (39, "B2026-04-B", False, ["reflective_artifact"]),
            (34, "B2026-05-A", False, ["scratch", "scratch"]),
            (30, "B2026-05-A", False, ["scratch"]),
            (25, "B2026-05-A", False, ["scratch", "pit"]),
            (20, "B2026-05-A", True, []),
            (16, "B2026-05-B", True, []),
            (12, "B2026-05-B", False, ["contamination"]),
            (8, "B2026-05-B", True, []),
            (3, "B2026-05-B", False, ["pit"]),
        ]
        reason_map = {
            "scratch": "表面线状划痕超过阈值，建议复核夹具接触和清洁流程",
            "pit": "局部点蚀/凹坑特征，建议复核来料与蚀刻工艺",
            "edge_chip": "边缘崩缺，建议检查搬运和定位机构",
            "reflective_artifact": "强反光导致疑似伪缺陷，建议降低照明或增加偏振",
            "contamination": "污染/异物遮挡，建议清洁样品后复检",
        }
        history: list[dict[str, Any]] = []
        for index, (days_ago, batch_id, quality, defect_types) in enumerate(patterns, start=1):
            detected_at = today - timedelta(days=days_ago)
            defects = [
                {
                    "defect_type": defect_type,
                    "position": [round(80.0 + index * 3.7, 1), round(120.0 + offset * 24.5, 1)],
                    "size": round(18.0 + index * 1.8 + offset * 4.0, 1),
                    "confidence": round(0.82 + (offset * 0.04), 2),
                    "region": "ROI-C" if defect_type == "scratch" else "ROI-B",
                    "is_qualified": False,
                    "is_ignored": False,
                    "reason": reason_map.get(defect_type, "检测到异常缺陷"),
                }
                for offset, defect_type in enumerate(defect_types)
            ]
            history.append(
                {
                    "sample_id": f"DEMO-{detected_at:%Y%m%d}-{index:03d}",
                    "batch_id": batch_id,
                    "tray_id": batch_id,
                    "position": index,
                    ...
                    "defects_num": len(defects),
                    "defects": defects,
                    "parsed_result": {
                        "rule_reason": defects[0]["reason"] if defects else "未发现超过阈值的缺陷",
                        "severity": "high" if len(defects) >= 2 else ("medium" if defects else "normal"),
                    },
                }
            )
        return history
