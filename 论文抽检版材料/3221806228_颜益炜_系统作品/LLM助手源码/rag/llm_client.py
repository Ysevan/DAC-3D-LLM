...
class OpenAICompatibleProviderAdapter(ProviderAdapter):
    ...
    def _responses_endpoint(self) -> str:
        ...
        if not self.config.api_base_url:
            raise LLMConfigurationError("OpenAI-compatible provider requires DAC3D_LLM_API_BASE_URL.")
        return self.config.api_base_url.rstrip("/") + "/responses"
    ...
    def _generate_with_responses_api(self, client: Any, prompt: str) -> str:
        response = client.post(
            self._responses_endpoint(),
            headers=self._headers(),
            json=self._responses_payload(prompt),
        )
        response.raise_for_status()
        data = response.json()
        answer = self._extract_responses_text(data)
        if not answer:
            raise LLMProviderError(f"Responses API returned an empty response: {data}")
        return answer
        ...
        try:
            ...
            with httpx.Client(timeout=self.config.timeout_seconds) as client:
                try:
                    ...
                    return self._generate_with_responses_api(client, prompt)
        except Exception as exc:  # pragma: no cover - network guarded
        ...
        if not answer:
            try:
                ...
                raise LLMProviderError(
                    f"OpenAI-compatible provider returned an empty chat response and "
                    f"Responses API fallback failed: {exc}"
                ) from exc
        return answer
    ...
    def stream_generate(
        ...
        *,
        ...
        try:
            ...
            if not yielded:
                with httpx.Client(timeout=self.config.timeout_seconds) as client:
                    answer = self._generate_with_responses_api(client, prompt)
                for idx in range(0, len(answer), 40):
                    yield answer[idx : idx + 40]
        except Exception as exc:  # pragma: no cover - network guarded
