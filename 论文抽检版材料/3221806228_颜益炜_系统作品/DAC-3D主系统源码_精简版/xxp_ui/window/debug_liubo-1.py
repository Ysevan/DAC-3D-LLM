  ...
  # 底面对焦
    ...
    def xy_scan1_thread(self, xy_pos):
        ...
        with self.condition:
            ...
            df = pd.read_csv(r"C:\Users\Administrator\Desktop\工装盘焦面11111111.csv", header=None,encoding="gbk")#指定文件编码参数
...
#             self.queue.put((frame,number))
        ...
        if not self.timestatus:
            ...
            #             rf"{self.cam3_path}\{self.pos_z - self.focus_pos}.bmp")
# 三个相机显示
# 核心：保持宽高比的缩放函数（适配QLabel，不裁剪、不变形）
def scale_pixmap_keep_aspect_ratio(pixmap: QPixmap, target_label) -> QPixmap:
    """
    缩放QPixmap，保持宽高比，完整适配目标QLabel的尺寸
    :param pixmap: 原始QPixmap对象
    :param target_label: 要显示图片的QLabel控件
    :return: 缩放后的QPixmap对象（可直接设置到QLabel）
    """
    if pixmap.isNull() or target_label.size().isEmpty():
        return QPixmap()

    # 关键：基于QLabel实际尺寸缩放，保持宽高比+平滑显示
    return pixmap.scaled(
        target_label.size(),  # 适配QLabel当前尺寸，而非固定比例
        Qt.KeepAspectRatio,  # 强制保持图片原始宽高比，不拉伸变形
        Qt.SmoothTransformation  # 平滑缩放，提升低分辨率图片显示效果
    )

    # 你的原有变量：frame（相机捕获的灰度帧）、number（相机编号）、各QLabel（label_8/9/10/13）
    height, width = frame.shape
    # 注意：原代码中QImage的第四个参数（bytesPerLine）传入width是正确的（灰度图每个像素1字节）
    ...
    if number == 'DA3827093':
        ...
        # 改造点1：先转为QPixmap，再调用保持宽高比函数，适配label_9尺寸
        pixmap = QPixmap.fromImage(mirrored_img)
        scaled_pixmap = scale_pixmap_keep_aspect_ratio(pixmap, self.label_9)
        # 改造点2：直接设置缩放后的Pixmap，无需手动指定宽高
        self.label_9.setPixmap(scaled_pixmap)
        # 可选优化：让图片在Label中居中显示，视觉效果更好
        ...
        # 同样适配label_10尺寸，保持宽高比
        pixmap = QPixmap.fromImage(rotated_img)
        scaled_pixmap = scale_pixmap_keep_aspect_ratio(pixmap, self.label_10)
        self.label_10.setPixmap(scaled_pixmap)
        self.label_10.setAlignment(Qt.AlignCenter)

    elif number == 'DA3562103':
        # 直接显示，保持宽高比适配label_8
        pixmap = QPixmap.fromImage(q_img)
        scaled_pixmap = scale_pixmap_keep_aspect_ratio(pixmap, self.label_8)
        self.label_8.setPixmap(scaled_pixmap)
        self.label_8.setAlignment(Qt.AlignCenter)
        # 原有保存图片功能保留
        cv2.imwrite('frame.jpg', frame)

    elif number == 'DA6630523':
        # 直接显示，保持宽高比适配label_13
        pixmap = QPixmap.fromImage(q_img)
        scaled_pixmap = scale_pixmap_keep_aspect_ratio(pixmap, self.label_13)
    ...
    def resizeEvent(self, event):
        ...
        # 定义一个辅助函数，重新更新单个Label的图片
        def update_camera_label(label, original_q_img, transform=None):
            if label.pixmap().isNull() or original_q_img.isNull():
                return
            # 应用变换（翻转/旋转，若无则直接使用原始q_img）
            if transform:
                processed_img = original_q_img.transformed(transform)
            else:
                processed_img = original_q_img
            # 重新缩放并设置
            pixmap = QPixmap.fromImage(processed_img)
            scaled_pixmap = scale_pixmap_keep_aspect_ratio(pixmap, label)
