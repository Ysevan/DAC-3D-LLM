    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = sum(ch[x] for x in f)
                ...
                {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect,PG_Detect}):
            # 如果是 PG_Detect，前几项是特征层通道，最后一项是先验图的分支
            if m is PG_Detect:
                # f 应该是类似 [19, 22, 25, 28, 29]
                # 我们取出前四个特征层的通道数，最后一项（PriorBypass）设为 1（代表先验图通道）
                args.append([ch[x] if isinstance(ch[x], int) else 1 for x in f])
            else:
            ...
            # 继承 legacy 属性确保兼容性
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, PG_Detect}:
                m.legacy = legacy
            ...
            c2 = ch[f[-1]]
        elif m is PriorBypass:
            args = [ch[f],*args]
        elif m is CoDA:
            args = [ch[f],*args]
        elif m is MiLKConvAttn:
            args = [ch[f],*args]
        elif m is EnhancedEMA:
            args = [ch[f],*args]
        elif m is SelfAttentionSEBlock:
            args = [ch[f],*args]
        elif m is FullyDynamicSEBlock:
            args = [ch[f],*args]
        elif m is DACModule:
            args = [ch[f],*args]
        elif m is SPRSA:
            args = [ch[f],*args]
        elif m is CGHalfConv_GS:
            args = [ch[f],*args]
        elif m is CoordAttMeanMax:
            args = [ch[f],ch[f],*args]
        elif m is MultiScaleFreqDenoise:
            args = [ch[f],*args]
        elif m is GTPStem:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # 如果配置文件里写了输出通道
                c2 = make_divisible(c2 * width, 8)
            args = [c1, c2] # 传入 input_channel, output_channel
        
        elif m is LCA:
            c_x = ch[f[0]] # 第一路输入通道
            c_y = ch[f[1]] # 第二路输入通道
            c2 = args[0]   # YAML 中填写的输出通道
            if c2 != nc:
                c2 = make_divisible(c2 * width, 8)
            args = [c_x, c_y, c2, *args[1:]]
        
        
        elif m is FreqAttnPlus:
            args = [ch[f],*args]
        elif m is InterRowColSelfAttention:
            args = [ch[f],*args]
