...
class DAC3DAssistant:
    ...
    def handle_message(
        ...
        try:
            ...
            inspection_history_response = self._handle_inspection_history_if_requested(clean_message)
            if inspection_history_response is not None:
                return inspection_history_response

            machine_response = self._handle_machine_analysis_if_requested(clean_message)
    ...
    def stream_message(
        ...
        try:
            ...
            inspection_history_response = self._handle_inspection_history_if_requested(clean_message)
            if inspection_history_response is not None:
                yield from self._emit_buffered_response(inspection_history_response)
                return

            machine_response = self._handle_machine_analysis_if_requested(clean_message)
            if machine_response is not None:
                yield from self._emit_buffered_response(machine_response)
                return
    ...
    def _handle_inspection_history_if_requested(self, message: str) -> AssistantResponse | None:
        """Route recent defect/sample trend questions to DAC-3D inspection history."""
        if not self._looks_like_inspection_history_request(message):
            return None

        months = 1 if "上个月" in message or "last month" in message.lower() else 3
        summary = self.dac3d_client.get_inspection_history_summary(months=months)
        return AssistantResponse(
            intent="inspection_history",
            answer=self._inspection_history_answer(summary),
            parsed_result=summary,
        )

    @staticmethod
    def _looks_like_inspection_history_request(message: str) -> bool:
        """Detect defect trend/history requests before operation parsing."""
        text = message.lower()
        time_terms = (
            "最近三个月",
            "三个月",
        ...
        equipment_only_terms = ("设备运行", "报警", "告警", "alarm")
        return (
            any(term in text for term in time_terms)
            and any(term in text for term in defect_terms)
            and any(term in text for term in analysis_terms)
            and not (
                any(term in text for term in equipment_only_terms)
                and not any(term in text for term in ("瑕疵", "缺陷", "样品", "批次", "defect", "sample"))
            )
        )

    @staticmethod
    def _inspection_history_answer(summary: dict[str, Any]) -> str:
        """Format the recent inspection-history trend summary for the operator."""
        source = str(summary.get("source") or "unknown")
        source_label = "DAC-3D 主系统历史结果" if source == "status_file" else "本地演示历史检测数据"
        months = int(summary.get("time_range_months") or 3)
        sample_count = int(summary.get("sample_count") or 0)
        qualified_count = int(summary.get("qualified_count") or 0)
        defective_count = int(summary.get("defective_sample_count") or 0)
        defect_total = int(summary.get("defect_total") or 0)
        lines = [
            f"已读取{source_label}，汇总最近{months}个月检测情况：共{sample_count}个样品，合格{qualified_count}个，不合格{defective_count}个，累计缺陷{defect_total}个。"
        ]

        top_defects = [item for item in (summary.get("top_defects") or []) if isinstance(item, dict)]
        if top_defects:
            defect_text = "、".join(
                f"{item.get('defect_type', 'unknown')} {item.get('count', 0)}次"
                for item in top_defects[:4]
            )
            lines.append(f"高频缺陷：{defect_text}。")
        else:
            lines.append("当前历史记录中没有统计到缺陷类型。")

        high_risk_batches = [
            item for item in (summary.get("high_risk_batches") or []) if isinstance(item, dict)
        ]
        if high_risk_batches:
            batch_lines = []
            for item in high_risk_batches[:3]:
                rate = float(item.get("defective_rate") or 0.0)
                batch_lines.append(
                    f"{item.get('batch_id', 'unknown')} 不合格{item.get('defective_count', 0)}/{item.get('sample_count', 0)}，比例{rate:.0%}"
                )
            lines.append(f"批次异常：{'；'.join(batch_lines)}。")
            lines.append("结论：存在批次聚集现象，建议优先复核高风险批次的原图、缺陷框、夹具接触面、清洁流程和照明设置。")
        elif defective_count:
            lines.append("结论：有零散不合格样品，但暂未形成明显批次聚集。建议继续观察后续样品。")
        else:
            lines.append("结论：最近历史样品整体稳定，暂未看到明显高频缺陷或批次异常。")

        if source != "status_file":
            lines.append("说明：当前主系统未发布足够的真实 result_history，因此这里使用本地演示历史数据；真实上线时可替换为 DAC-3D 历史结果或数据库记录。")
        return "\n".join(lines)

    def _handle_machine_analysis_if_requested(self, message: str) -> AssistantResponse | None:
        """Route equipment history/alarm analysis questions to the machine Agent."""
        if not self._looks_like_machine_analysis_request(message):
            return None
        if MachineAgentService is None:
            return AssistantResponse(
                intent="machine_agent",
                answer="设备异常分析模块当前不可用，请检查 machine_agent 依赖是否完整。",
            )

        payload = MachineAgentService().chat(message)
        return AssistantResponse(
            intent="machine_agent",
            answer=str(payload.get("answer") or "已完成设备异常分析。"),
            parsed_result=payload,
        )

    @staticmethod
    def _looks_like_machine_analysis_request(message: str) -> bool:
        """Detect trend/history/alarm requests before generic operation parsing."""
        text = message.lower()
        time_terms = (
            "最近三个月",
            "三个月",
        ...
        summary_terms = ("总结", "汇总", "分析", "梳理", "归因", "有没有", "怎么样", "summary", "summarize")
        return (
            any(term in text for term in time_terms)
            and any(term in text for term in analysis_terms)
            and any(term in text for term in summary_terms)
        )
    ...
    def _attach_runtime_context_to_command(
        ...
        elif str(self.dac3d_client.endpoint).startswith("file://"):
            runtime_mode = "status_file"
        elif self.dac3d_client.mock_mode:
            runtime_mode = "mock"
        else:
            runtime_mode = "live"
        if action in {"scan", "start_online_scan", "start_offline_detection"} and runtime_mode in {"mock", "status_file"}:
            warning = "当前未确认真实设备在线，本次仅生成预览，执行前需确认 DAC-3D 主系统和设备连接状态。"
            if warning not in command_warnings:
                command_warnings.append(warning)
            preview.setdefault("warnings", [])
            if warning not in preview["warnings"]:
