...
class DAC3DAgentChatAdapter:
    ...
    def handle_message(
        ...
        *,
        ...
        direct_response = self.runtime.assistant._handle_inspection_history_if_requested(message)
        if direct_response is not None:
            self._record_agent_trace(
                session_id=session_id,
                message=message,
                response=direct_response,
                context_bundle={"routing": "deterministic_inspection_history"},
                event_type="agent_chat",
            )
            self._remember_turn(session_id, message, direct_response)
