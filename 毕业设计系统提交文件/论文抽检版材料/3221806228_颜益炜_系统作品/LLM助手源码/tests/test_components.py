...
def test_dac3d_client_prefers_real_history_over_demo_history(tmp_path) -> None:
    """Inspection trend summaries should prefer host-published result_history."""
    status_file = tmp_path / "dac3d_runtime_status.json"
    status_file.write_text(
        json.dumps(
            {
                    ...
                    "result_history": [
                        {
                            ...
                            "quality_label": "不合格",
                            "defects_num": 1,
                            "defects": [{"defect_type": "real_scratch"}],
                        },
                        {
                            "sample_id": "REAL-002",
                            "batch_id": "REAL-BATCH",
    ...
    summary = client.get_inspection_history_summary(months=3)

    assert summary["source"] == "status_file"
    assert summary["sample_count"] == 2
    assert summary["defect_type_counts"] == {"real_scratch": 1}
    assert summary["records"][0]["sample_id"] == "REAL-001"
