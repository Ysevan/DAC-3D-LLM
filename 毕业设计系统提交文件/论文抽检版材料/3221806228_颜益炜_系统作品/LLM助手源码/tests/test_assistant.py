...
def test_machine_history_analysis_is_not_misrouted_as_operation(tmp_path: Path) -> None:
    """Historical abnormality questions should use the machine Agent, not scan preview."""
    config = make_config(tmp_path)
    assistant = DAC3DAssistant.create(config, rebuild_kb=True)

    response = assistant.handle_message("帮我总结最近三个月的检测/设备异常情况，有没有高频异常或明显趋势？")

    assert response.intent == "machine_agent"
    assert response.command_preview is None
    assert response.parsed_result is not None
    tool_names = [item["name"] for item in response.parsed_result["tool_calls"]]
    ...
    assert response.parsed_result["summary_result"] is not None
    assert response.parsed_result["abnormal_result"] is not None


def test_inspection_history_analysis_uses_demo_result_history(tmp_path: Path) -> None:
    """Recent defect trend questions should summarize sample history, not create commands."""
    config = make_config(tmp_path)
    assistant = DAC3DAssistant.create(config, rebuild_kb=True)

    response = assistant.handle_message("帮我总结最近三个月的检测缺陷趋势，有没有高频瑕疵或批次异常？")

    assert response.intent == "inspection_history"
    assert response.command_preview is None
    assert response.parsed_result is not None
    assert response.parsed_result["source"] == "demo_inspection_history"
    assert response.parsed_result["sample_count"] > 0
    assert response.parsed_result["top_defects"]
    assert response.parsed_result["high_risk_batches"]
    assert "最近3个月" in response.answer
    assert "高频缺陷" in response.answer
...
def test_scan_preview_warns_when_real_device_is_not_confirmed(tmp_path: Path) -> None:
    """Mock/file-bridge previews should remind operators to confirm real hardware."""
    config = make_config(tmp_path)
    assistant = DAC3DAssistant.create(config, rebuild_kb=True)

    response = assistant.handle_message("我想扫描一个 25mm×25mm 的楔形滤光片，步长 10 微米")

    assert response.intent == "operation"
    assert response.command_preview is not None
    warnings = response.command_preview["warnings"]
    assert any("当前未确认真实设备在线" in warning for warning in warnings)
    assert "当前未确认真实设备在线" in response.answer
