...
try:
    ...
    gclib = None
    GCLIB_IMPORT_ERROR = e
else:
    GCLIB_IMPORT_ERROR = None
from PyQt5.QtCore import QTimer, QDateTime, QEvent, pyqtSlot
...
try:
    ...
    CAMERA_IMPORT_ERROR = e

    class CameraFactory:
        def __init__(self):
            raise RuntimeError(f'相机SDK不可用：{CAMERA_IMPORT_ERROR}')
else:
...
try:
    ...
    class ZAUXDLL:
        def __init__(self):
            raise RuntimeError(f'ZMC SDK不可用：{ZAUX_IMPORT_ERROR}')
else:
    ZAUX_IMPORT_ERROR = None
try:
    from window.control.light import lightControllor
except Exception as e:
    LIGHT_IMPORT_ERROR = e

    class lightControllor:
        def __init__(self):
            raise RuntimeError(f'光源控制不可用：{LIGHT_IMPORT_ERROR}')
else:
    LIGHT_IMPORT_ERROR = None
from window.control.getposxy import getposxy
#相机全局变量
CAM_BACK = "DA3827093"  # 焦后
CAM_FRONT = "DA3562117" # 焦前
CAM_FOCU = "DA3562103"  # 焦面
...
class OfflineGalil:
    ...
    def GOpen(self, address):
        print(f'[offline] skip Galil GOpen: {address}')
        return ''
    ...
    def GVersion(self):
        return 'offline-galil'

    def GCommand(self, command):
        command = command.strip()
        if command.startswith('pPOSX='):
            self.pos_x = float(command.split('=', 1)[1])
            return ''
        if command.startswith('pPOSY='):
            self.pos_y = float(command.split('=', 1)[1])
            return ''
        if command == 'TPA':
            return f'TPA: {self.pos_x * 6400}'
        if command == 'TPB':
            return f'TPB: {self.pos_y * 6400}'
        if command in ('pMOVEX=', 'pMOVEY='):
            return '0'
        print(f'[offline] Galil command: {command}')
        return '0'
...
class OfflineZmc:
    ...
    def __getattr__(self, name):
        def offline_method(*args, **kwargs):
            return 0
        return offline_method

    def ZAux_OpenEth(self, address):
        print(f'[offline] skip ZMC OpenEth: {address}')
        return 0

    def ZAux_Direct_GetMpos(self, axis):
        return 0, OfflineValue(self.pos_z)

    def ZAux_Direct_GetIfIdle(self, axis):
        return 0, OfflineValue(1)

    def ZAux_Direct_Single_Move(self, axis, distance):
        self.pos_z += distance / 1000
        return 0

    def ZAux_Direct_Single_MoveAbs(self, axis, position):
        self.pos_z = position / 1000
        return 0


class OfflineLight:
    def light15control(self, value):
        print(f'[offline] light15control({value})')

    def light60control(self, value):
        print(f'[offline] light60control({value})')


if gclib is None:
    print(f'Galil gclib 不可用，启用离线模式：{GCLIB_IMPORT_ERROR}')
    gclib = OfflineGclib()
...
class Debug_UI(QtWidgets.QMainWindow):
    ...
    def __init__(self,debug_ui_queue=None,ui_debug_queue=None,debug_image_queue=None,image_debug_queue=None,image_queue = None,mode='mainUICall', offline_hardware=False):
        super(Debug_UI, self).__init__()
        self.img_get_finished = False
        self.img_flag = None
        ...
        self.save_path_layerScan = str(RUNTIME_ROOT / 'layerscan')
        self.save_path_snakeScan = str(RUNTIME_ROOT / 'snakescan')
        self.save_path_triggerScan = str(RUNTIME_ROOT / 'trigger')

        self.layerscan_queue = queue.Queue()

        if offline_hardware:
            self.init_offline_hardware()
        else:
            self.Init_hardware()
        ...
        self.camera_save = 0
        # 按钮绑定
        uic.loadUi(str(PROJECT_ROOT / 'ui' / 'debug.ui'), self)  # 加载UI文件
        ...
        # self.pushButton_3.clicked.connect(self.stagestatus)
        ...
        if self.offline_hardware:
            self.model = None
            print('离线硬件模式，跳过自动对焦模型加载')
        else:
            self.model = SwinFusion_single().to(self.device)
            model_path = PROJECT_ROOT / 'bestxinxinxin.pth'
            if model_path.exists():
                self.model.load_state_dict(
                    torch.load(str(model_path), weights_only=False)['model_state_dict'])
            else:
                print(f'模型文件不存在，跳过加载：{model_path}')
    ...
    def Init_hardware(self):
        self.offline_hardware = False
        self.g = OfflineGalil()
        self.Zmc = OfflineZmc()
        self.light = OfflineLight()
        self.cam = None
        self.camList = []
        try:
            print('正在初始化硬件')
            ...
            custom_order = {CAM_BACK: 0, CAM_FRONT: 1, CAM_FOCU: 2}
            ...
            self.camList[2]['cam'].MV_CC_SetFloatValue("ExposureTime", 7000)
            #self.camList[3]['cam'].MV_CC_SetFloatValue("ExposureTime", 1000)
            ...
            self.offline_hardware = True
            print(f'硬件初始化出现错误：{e}')
            print('已切换为离线硬件占位，仅用于打开和调试UI')

    def init_offline_hardware(self):
        self.offline_hardware = True
        self.g = OfflineGalil()
        self.Zmc = OfflineZmc()
        self.light = OfflineLight()
        self.cam = None
        self.camList = []
        print('离线测试模式：跳过硬件初始化')
# 蛇形扫描
    def scan_xy(self):
        delaytime=1

        self.save_path_snakeScan = str(RUNTIME_ROOT / 'snakescan' / (self.date_folder + '-' + self.time_folder))
  ...
  # 底面对焦
    def botton_focus(self):
        #复位初始化
        self.img_get_finished = True
        self.camera_save = 0
        self.thread_run_status = 'idle'
        self.save_path_snakeScan = str(RUNTIME_ROOT / 'snakescan' / (self.date_folder + '-' + self.time_folder))
        os.makedirs(self.save_path_snakeScan, exist_ok=True)
        os.makedirs(self.save_path_snakeScan + '/焦前', exist_ok=True)
        os.makedirs(self.save_path_snakeScan + '/焦后', exist_ok=True)
    ...
    def xy_scan1_thread(self, xy_pos):
        ...
        with self.condition:
            ...
            df = pd.read_csv(r"C:\Users\Administrator\Desktop\工装盘焦面2.csv", header=None)#指定文件编码参数
            a_column_data=df.iloc[:, 1].tolist()
            ...
            for (i, pos) in enumerate(xy_pos):
                ...
                if i % 12 == 0:
                    ...
                    self.status ='snake_scan'
                    self.takePhoto()
                    # while not self.img_get_finished:
                    #     pass
                    # self.img_get_finished = False
                    time.sleep(1)
                    print("拍了")
                    self.move_z_up(a_column_data[i])
                    self.z_status()
                    sleep(0.5)
                    self.camera_save=4
                    self.status ='snake_scan'
            ...
            try:
                ...
                if item:
                    if isinstance(item,bool):
                        ...
                        value = tenengrad(detect_circle_and_extract_inside(item['CAM_BACK'])[0])
...
#    层扫
    def zscan(self):
        ...
        base_path = str(RUNTIME_ROOT / "layerscan")  # 指定基础路径
    ...
    def auto_focus_pic_take(self):
        ...
        base_path = str(RUNTIME_ROOT / "auto_focus_sample")  # 指定基础路径
    ...
    def dp_focus_get_img(self):
        while True:
            if len(self.dp_focus_img_list) >= 3:
                ...
                im1 = self.dp_focus_img_list['CAM_BACK']
                im2 = self.dp_focus_img_list['CAM_FRONT']
                im3 = self.dp_focus_img_list['CAM_FOCU']
...
#             self.queue.put((frame,number))
        ...
        if self.camera_save == 1:
            if number == CAM_BACK:
                ...
                if self.status == 'trigger':
                    ...
                    overwrite_dir = str(PROJECT_ROOT / 'Algorithm' / 'demo_img' / 'demo')
                    if not os.path.exists(overwrite_dir):
                        os.makedirs(overwrite_dir)
                    overwrite_save_path = rf"{overwrite_dir}\焦后.bmp"
                    if os.path.exists(overwrite_save_path):
                        os.remove(overwrite_save_path)
                    img.save(overwrite_save_path)
        ...
        if self.camera_save == 2:
            if number == CAM_FRONT:
                ...
                if self.status == 'trigger':
                    ...
                    overwrite_dir = r"F:\福特科\xxp_ui\Algorithm\demo_img\demo"
                    if not os.path.exists(overwrite_dir):
                        os.makedirs(overwrite_dir)
                    overwrite_save_path = rf"{overwrite_dir}\焦前.bmp"
                    if os.path.exists(overwrite_save_path):
                        os.remove(overwrite_save_path)
                    flipped_img.save(overwrite_save_path)
        ...
        if self.camera_save == 3:
            if number == CAM_FOCU:
                ...
                if self.status == 'trigger':
                    ...
                    if not os.path.exists(overwrite_dir):
                        os.makedirs(overwrite_dir)
                    overwrite_save_path = rf"{overwrite_dir}\焦面.bmp"
                    if os.path.exists(overwrite_save_path):
                        os.remove(overwrite_save_path)
                    img.save(overwrite_save_path)
                # elif self.status == 'z_scan':
        ...
        if self.camera_save == 4:
            ...
            if self.status == 'snake_scan':
                ...
                img = frame
                if number == CAM_BACK:
                    # DA3827093 → 焦后，不翻转（trigger已翻转，这里保存镜像即原始图）
                    pic_path = rf"{self.save_path_snakeScan}\焦后\{self.photo_index}.bmp"
                    Image.fromarray(img).save(pic_path)
                if number == CAM_FRONT:
                    # DA3562117 → 焦前，水平镜像翻转（原错误已修复）
                    pic_path = rf"{self.save_path_snakeScan}\焦前\{self.photo_index}.bmp"
                    img_pil = Image.fromarray(img)  # 将numpy数组转为PIL Image
                    flipped_img = img_pil.transpose(Image.FLIP_LEFT_RIGHT)  # 水平镜像翻转
                    flipped_img.save(pic_path)  # 保存翻转后的图像
                    img = np.array(img)  # 转回numpy数组
                if number == CAM_FOCU:
                    # DA3562103 → 焦面，水平翻转（镜像）
                    pic_path = rf"{self.save_path_snakeScan}\焦面\{self.photo_index}.bmp"
                    img = Image.fromarray(img)
                    img.save(pic_path)
                    img = np.array(img)  # 转回 numpy 用于后续处理
                ...
                #
                #     # 1. 确定当前表面的类型 (用于子进程区分处理)
                #     # 假设 indexofsurface 切换逻辑：0 为 Top, -1 为 Bottom
                #     s_type = "Top" if self.indexofsurface == false else "Bottom"
                #     print(s_type,'---------------------------------------')
                #     self.image_data['surface_type'] = s_type
                #
                #     # 2. 将当前表面数据存入临时缓冲区 (必须使用 .copy()，避免 self.image_data={} 影响数据)
                #     self.surface_buffer.append(self.image_data.copy())
                #
                #     # --- 状态重置（为下一个表面做准备） ---
                #     self.indexofsurface = ~self.indexofsurface  # 翻转表面标志
                #     self.camera_save_index = 0
                #     self.camera_save = 0
                #     self.status = None
                #     self.img_get_finished = True
                #     self.image_data = {}
                #
                #     # 3. 核心打包逻辑：缓冲区凑齐 2 个表面（无论顺序是上下还是下上）
                #     if len(self.surface_buffer) == 2:
                #
                #         self.xxp_pos += 1  # 完成一个元件，编号加 1
                #         print(self.xxp_pos, '---------------------------------------')
                #         # 结果图片的基础保存路径（不带后缀）
                #         res_base = rf"{self.save_path_snakeScan}\检测结果\pos_{self.xxp_pos}_{self.pos_x}_{self.pos_y}"
                #
                #         # 构建大礼包
                #         bundle = {
                #             'pos': self.xxp_pos,
                #             'tray_id': self.Tray_id,
                #             'surfaces': self.surface_buffer,  # 包含两个面数据的列表
                #             'result_base_path': res_base
                #         }
                #
                #         if self.debug_image_queue:
                #             try:
                #                 # 使用 block=False，如果队列满了立即报错而不是卡死
                #                 self.debug_image_queue.put(bundle, block=False)
                #                 print(f"--- [发送成功] 元件 {self.xxp_pos} ---")
                #             except queue.Full:
                #                 print("!!! [警告] 队列已满，  子进程处理太慢，该帧被跳过，防止卡死 !!!")
                #         self.surface_buffer = []

                    # # 4. 满盘检测
                    # if self.xxp_pos >= 144:
                    #     # 这里可以触发 UI 的完成消息
                    #     print("本盘 144 个样本已全部发送至处理队列")
                if self.camera_save_index>=3:
                    self.indexofsurface = ~self.indexofsurface
                    self.xxp_pos += 1
                    if self.debug_image_queue:
                        result_pic_path = rf"{self.save_path_snakeScan}\检测结果\{self.pos_x},{self.pos_y}.jpg"
                        self.image_data['result_pic_path'] = result_pic_path
                        self.image_data['pos'] = self.xxp_pos
                        self.debug_image_queue.put(self.image_data)
                        self.image_data = {}
                    if self.xxp_pos>=144:
                        self.xxp_pos=0
...
# 三个相机显示
        ...
        if number == CAM_BACK:
            # DA3827093 不进行翻转，直接显示
            self.label_9.setPixmap(QPixmap.fromImage(q_img).scaled(width // 5, height // 5))
        if number == CAM_FRONT:
            transform = QTransform().scale(-1, 1)
            mirrored_img = q_img.transformed(transform)
            self.label_10.setPixmap(QPixmap.fromImage(mirrored_img).scaled(width // 5, height // 5))
        if number == CAM_FOCU:
           self.label_8.setPixmap(QPixmap.fromImage(q_img).scaled(width // 5, height // 5))
           cv2.imwrite('frame.jpg',frame)
        #if number == 'DA6630523':
           # self.label_13.setPixmap(QPixmap.fromImage(q_img).scaled(width // 5, height // 5))
           # cv2.imwrite('frame.jpg',frame)
    ...
    def receive_img(self):
        ...
        while True:
            # try:
            #     item = self.layerscan_queue.get(timeout=1)
            #     if item[0]==CAM_BACK:
            #         index += 1
            #         print(index)
            ...
            try:
                # 先取第一条数据（触发超时）
                item = self.layerscan_queue.get(timeout=0.5)
                # 嵌套循环：取出所有可用数据
                while True:
                    if item[0] == CAM_FOCU:
                        index += 1
                    ...
                    # 检查队列是否为空，空则退出内层循环
                    if self.layerscan_queue.empty():
                        break
                    # 非阻塞取下一条数据
                    item = self.layerscan_queue.get_nowait()
            except queue.Empty:
                # 超时无数据，跳过
                pass
            except Exception as e:
