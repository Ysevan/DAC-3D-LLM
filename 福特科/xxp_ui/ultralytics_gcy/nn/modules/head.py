...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect","PG_Detect"
...
class PG_Detect(Detect):
    def __init__(self, nc=80, ch=()):
        feat_ch = ch[:-1] 
        super().__init__(nc, feat_ch) 
        self.prior_alphas = nn.Parameter(torch.ones(self.nl, 1, 1, 1) * 0.2)
        self.prior_alphas.data[0] = 0.5 
        # P5层负责大物体，Splash已经够好了，保持 0.2 或更小
        self.prior_alphas.data[3] = 0.1

    def _apply_prior_guidance(self, x):
        """内部工具函数：负责将先验注入到分类分支"""
        p_map = x[-1]  # 剥离先验
        feats = x[:-1] # 剥离特征
        res = []
        for i in range(self.nl):
            p_resized = F.interpolate(p_map, size=feats[i].shape[-2:], mode='bilinear', align_corners=False)
            
            # 分别计算回归和分类
            b_out = self.cv2[i](feats[i])
            c_out = self.cv3[i](feats[i])
            
            # ✨ 核心注入
            c_out = c_out + (self.prior_alphas[i] * p_resized)
            
            res.append(torch.cat((b_out, c_out), 1))
        return res

    def forward(self, x):
        # 1. 首先应用先验引导，得到纯净的特征列表 [P2, P3, P4, P5]
        # 每个 Tensor 形状均为 [B, 67, H, W]
        x_guided = self._apply_prior_guidance(x)

        # 2. 接管原版 Detect 的所有判断分支
        if getattr(self, 'end2end', False):
            # 如果是端到端模式，必须调用专门的 forward_end2end
            # 但注意：forward_end2end 内部会再次调用 cv2/cv3，这会产生冲突
            # 工业检测通常不建议开启 end2end，建议在 YAML 或训练配置中关闭它
            return self.forward_end2end(x_guided)

        if self.training:
            return x_guided
        
        # 推理模式
        y = self._inference(x_guided)
        return y if self.export else (y, x_guided)
