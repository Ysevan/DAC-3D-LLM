...
Agent 工作台现在包含本地 Goal Tracker、Agent Task Board、Automation Planner、Workflow Templates、Artifact Store、Event Queue、Repo Context Map、Git Workspace Context、Agent Registry、Conversation Thread Store、Browser Context Store、Tool Marketplace Catalog、Agent Deployment Catalog、Agent Labeling Queue、Agent Performance Store、Verification Feedback Runner、Review Handoff Queue、Code Symbol Navigator、Agent Checkpoint Store、Agent Observability Snapshot 和 Agent Shared State Store。目标以 JSON 写入 `.tmp/conversation_memory/agent_goals.json`，支持创建目标、追加进度和标记完成；任务卡以 JSON 写入 `.tmp/conversation_memory/agent_task_board.json`，可从 workflow preview 生成，并在 `backlog / ready / in_progress / blocked / done` 状态列之间移动；自动化计划以 JSON 写入 `.tmp/conversation_memory/agent_automations.json`，记录 daily / weekly / interval / manual 计划、下一次运行时间和外部 runner 回写的运行摘要；工作流模板以 JSON 写入 `.tmp/conversation_memory/agent_workflows.json`，可把一次 workflow preview 固化成可复用 DAG 节点和边；Artifact Store 用 `.tmp/conversation_memory/agent_artifacts.json` 保存索引，并把 markdown、json、text、diff、log 或 link 内容写入 `.tmp/conversation_memory/artifacts/`，用于保存 Agent 输出、执行摘要、日志片段和可复用上下文；Event Queue 用 `.tmp/conversation_memory/agent_events.json` 保存 `queued / claimed / completed / failed / cancelled` 事件，支持按优先级和 scheduled time 查询、claim 和回写执行结果；Repo Context Map 用 `.tmp/conversation_memory/repo_context_map.json` 保存静态项目地图，记录模块、文件角色、Python symbols 和 FastAPI route 摘要；Git Workspace Context 只读汇总当前 branch、upstream、ahead/behind、changed files、diff stat、recent commits 和 worktrees；Agent Registry 用 `.tmp/conversation_memory/agent_registry.json` 保存 coordinator/specialist 的 role、capabilities、tools、triggers 和 handoff 元数据；Conversation Thread Store 用 `.tmp/conversation_memory/agent_threads.json` 保存多 Agent thread、参与者、消息和 shared context 引用；Browser Context Store 用 `.tmp/conversation_memory/agent_browser_contexts.json` 保存共享 URL、页面观察、截图/产物引用和 thread 关联；Tool Marketplace Catalog 用 `.tmp/conversation_memory/agent_tool_marketplace.json` 保存可复用工具包、所需上下文、示例提示和 workflow builder 元数据；Agent Deployment Catalog 用 `.tmp/conversation_memory/agent_deployments.json` 保存低代码 Agent app、workflow app、release 和入口元数据；Agent Labeling Queue 用 `.tmp/conversation_memory/agent_labeling.json` 保存 trace/对话样本、人工标签、评分和 JSONL 导出记录；Agent Performance Store 用 `.tmp/conversation_memory/agent_performance.json` 保存 latency、tokens、cost、quality score 和 tool-call count 等 LLMOps 指标。聊天中出现“目标：...”或 `goal:` 这类明确目标信号时，会自动把当前 session 的目标沉淀为可见条目。Web 设置面板的“Agent 目标”会调用 `GET /api/goals`、`POST /api/goals`、`POST /api/goals/{goal_id}/progress` 和 `POST /api/goals/{goal_id}/complete`；任务板 API 为 `GET/POST /api/agent/tasks`、`POST /api/agent/tasks/from-workflow` 和 `POST /api/agent/tasks/{task_id}/status`；自动化计划 API 为 `GET/POST /api/agent/automations`、`GET /api/agent/automations/due`、`POST /api/agent/automations/{automation_id}/status` 和 `POST /api/agent/automations/{automation_id}/runs`；工作流模板 API 为 `GET/POST /api/agent/workflows`、`POST /api/agent/workflows/from-preview` 和 `POST /api/agent/workflows/{workflow_id}/status`；Artifact Store API 为 `GET/POST /api/agent/artifacts` 和 `GET /api/agent/artifacts/{artifact_id}`，支持按 session、类型、tag 和关键词检索；Event Queue API 为 `GET/POST /api/agent/events`、`GET /api/agent/events/due`、`POST /api/agent/events/claim` 和 `POST /api/agent/events/{event_id}/status`；Repo Context Map API 为 `GET /api/agent/repo-map`、`POST /api/agent/repo-map/build` 和 `GET /api/agent/repo-map/search?q=...`；Git Workspace Context API 为 `GET /api/agent/git/status`；Agent Registry API 为 `GET/POST /api/agent/registry`、`GET /api/agent/registry/{agent_id_or_role}` 和 `POST /api/agent/registry/route`；Conversation Thread API 为 `GET/POST /api/agent/threads`、`GET /api/agent/threads/{thread_id}`、`POST /api/agent/threads/{thread_id}/messages` 和 `POST /api/agent/threads/{thread_id}/status`；Browser Context API 为 `GET/POST /api/agent/browser-contexts`、`GET /api/agent/browser-contexts/{context_id}`、`POST /api/agent/browser-contexts/{context_id}/observations` 和 `POST /api/agent/browser-contexts/{context_id}/status`；Tool Marketplace API 为 `GET/POST /api/agent/tool-marketplace`、`GET /api/agent/tool-marketplace/{entry_id_or_slug}` 和 `POST /api/agent/tool-marketplace/{entry_id_or_slug}/status`；Agent Deployment API 为 `GET/POST /api/agent/deployments`、`GET /api/agent/deployments/{deployment_id_or_slug}`、`POST /api/agent/deployments/{deployment_id_or_slug}/releases` 和 `POST /api/agent/deployments/{deployment_id_or_slug}/status`；Agent Labeling API 为 `GET/POST /api/agent/labeling`、`POST /api/agent/labeling/from-trace`、`GET /api/agent/labeling/{item_id}`、`POST /api/agent/labeling/{item_id}/labels`、`POST /api/agent/labeling/{item_id}/status` 和 `GET /api/agent/labeling/export`；Agent Performance API 为 `GET /api/agent/performance`、`POST /api/agent/performance/metrics`、`POST /api/agent/performance/from-trace`、`GET /api/agent/performance/summary` 和 `POST /api/agent/performance/{metric_id}/status`。这些能力用于把长任务目标、可执行任务卡、计划任务、可复用 DAG、共享产物、可恢复事件、项目结构上下文、Git 工作区上下文、agent 能力目录、threaded 协作上下文、shared browser context、tool marketplace、agent deployments、agent labeling datasets、agent performance metrics、验证反馈、评审交接、符号导航、恢复点、可观测摘要、scoped shared state 和每轮 Agent trace、Context Tree、Memory OS 一起纳入工作区视图；当前不会启动后台守护进程，真实执行可由未来 worker/CLI 读取 due items、event queue、agent registry、conversation threads、browser contexts、tool marketplace、agent deployments、agent labeling、agent performance、shared state 或 checkpoint 后再走统一 Agent 入口。

Verification Feedback Runner 已接入 Agent 工作台，用 `.tmp/conversation_memory/agent_verifications.json` 记录本地验证反馈。它提供 `python_compile`、`python_tests` 和 `ruff_check` 预设，运行结果会保存 status、exit code、耗时、stdout/stderr tail 和时间戳，供 Agent Workspace 回看“本轮改动验证到哪一步”。Web/API 入口为 `GET /api/agent/verifications/presets`、`POST /api/agent/verifications/run` 和 `GET /api/agent/verifications`，当前只接受内置 preset，不提供任意 shell 命令输入。

Review Handoff Queue 用 `.tmp/conversation_memory/agent_review_handoffs.json` 保存待评审工作包，可把任务、workflow、trace、变更文件、verification run、checklist 和评论串起来，供人工或 reviewer agent 继续判断。Web/API 入口为 `GET/POST /api/agent/reviews`、`GET /api/agent/reviews/{review_id}`、`POST /api/agent/reviews/{review_id}/comments` 和 `POST /api/agent/reviews/{review_id}/status`，支持 `pending / in_review / approved / needs_changes / rejected / archived` 状态流转。

Code Symbol Navigator 基于 Repo Context Map 的静态扫描结果，为 Python `class` / `def` / `async def` 提供轻量符号导航。`symbol_details` 会记录 name、kind、line、signature、path 和 module；`GET /api/agent/symbols?q=...&kind=function` 可按名称、签名、路径或模块查询定义位置，供 Agent 在改代码前快速定位上下文。

Agent Checkpoint Store 用 `.tmp/conversation_memory/agent_checkpoints.json` 保存工作流恢复点，记录 title、summary、session、task/workflow/event/review/trace 关联、state、tags 和历史事件。`POST /api/agent/checkpoints` 可保存一个 JSON 状态快照；`POST /api/agent/checkpoints/{checkpoint_id}/restore` 会把它标记为当前恢复点，方便长任务或多 Agent 接力继续，不会回滚文件或自动修改 DAC 命令状态。

Agent Observability Snapshot 聚合 `agent_traces.jsonl`、Event Queue、Verification Feedback、Review Handoff 和 Checkpoint Store 的运行信号，输出 recent traces、intent/tool-call 分布和 queued events、failed verifications、pending reviews、active checkpoints 这些 attention counters。Web/API 入口为 `GET /api/agent/observability?recent_trace_limit=20`，用于给工作台或后续 reviewer agent 快速判断当前系统状态。

Agent Shared State Store 用 `.tmp/conversation_memory/agent_shared_state.json` 保存 scoped JSON 状态，支持 `session / workflow / task / global` 四类 scope 和 namespace 隔离。`POST /api/agent/shared-state` 可按 key upsert 状态，`GET /api/agent/shared-state` 可按 session、scope、namespace、tag 或关键词检索，`GET /api/agent/shared-state/{state_id_or_key}` 可读取一个共享状态，用于多 Agent 之间传递当前 workflow、active context 或阶段性结果。

Agent Registry 用 `.tmp/conversation_memory/agent_registry.json` 保存多 Agent 工作区的能力目录。启动时会 seed 默认 coordinator、DAC-3D QA、Control、Result、Machine、Memory、Skill 和 Safety agents；`GET /api/agent/registry` 支持按 status、role、capability 或关键词检索，`POST /api/agent/registry` 可登记新的 specialist，`POST /api/agent/registry/route` 会根据 task 文本匹配 triggers/capabilities 并返回候选 agent，供后续 router/supervisor 做 handoff 决策。

Conversation Thread Store 用 `.tmp/conversation_memory/agent_threads.json` 保存多 Agent 协作 thread。`POST /api/agent/threads` 可创建包含 participants、shared_state_ids、artifact_ids 和 tags 的 thread；`POST /api/agent/threads/{thread_id}/messages` 追加 agent/tool/user 消息和工具调用摘要；`GET /api/agent/threads` 支持按 session、status、participant、tag 或关键词检索；`POST /api/agent/threads/{thread_id}/status` 可把 thread 标记为 paused、resolved 或 archived，方便后续 agent 接力恢复上下文。

Browser Context Store 用 `.tmp/conversation_memory/agent_browser_contexts.json` 保存共享浏览上下文记录，不会自动启动 Chrome。`POST /api/agent/browser-contexts` 创建 URL/thread 关联；`POST /api/agent/browser-contexts/{context_id}/observations` 追加页面标题、文本摘要、selector、截图路径或 artifact 引用；`GET /api/agent/browser-contexts` 支持按 session、thread、status、tag 或关键词检索；`POST /api/agent/browser-contexts/{context_id}/status` 可把上下文标记为 captured、stale 或 archived。

Tool Marketplace Catalog 用 `.tmp/conversation_memory/agent_tool_marketplace.json` 保存本地工具包目录。启动时会 seed DAC-3D Control、QA、Result、Machine Diagnostics、Memory OS、Skill System 和 Workspace Collaboration 等内置工具包；`GET /api/agent/tool-marketplace` 可按 status、category、provider、tag 或关键词检索，`POST /api/agent/tool-marketplace` 可登记新的工具包，`POST /api/agent/tool-marketplace/{entry_id_or_slug}/status` 可在 `available / installed / disabled / archived` 之间流转，供低代码 workflow builder 选择工具组合。

Agent Deployment Catalog 用 `.tmp/conversation_memory/agent_deployments.json` 保存低代码 Agent 应用和 workflow app 的部署目录。`GET/POST /api/agent/deployments` 可列出或登记部署条目，`GET /api/agent/deployments/{deployment_id_or_slug}` 可读取完整配置，`POST /api/agent/deployments/{deployment_id_or_slug}/releases` 可记录 release 版本、产物和验证运行引用，`POST /api/agent/deployments/{deployment_id_or_slug}/status` 可在 `draft / ready / deployed / paused / archived` 之间流转，供 workflow builder 把工具包、workflow 模板、Agent roles 和入口路径沉淀为可发布的本地应用目录。

Agent Labeling Queue 用 `.tmp/conversation_memory/agent_labeling.json` 保存 LLMOps 数据标注队列。它可以从 trace 创建待标注样本，记录 answer quality、intent correctness、tool-call correctness、memory action 等人工标签和 0-5 分评分；`GET /api/agent/labeling/export` 会返回 JSONL-ready 数据，供后续 eval case、人审样本或性能分析复用。

Agent Performance Store 用 `.tmp/conversation_memory/agent_performance.json` 保存 LLMOps 性能指标。`POST /api/agent/performance/metrics` 可记录手动指标，`POST /api/agent/performance/from-trace` 可从 trace 抽取 latency、token、tool-call count、quality score 或 cost 指标，`GET /api/agent/performance/summary` 会按 metric/category 汇总 count、avg、min 和 max，供低代码工作台做性能分析。

Agent Grounding Store 用 `.tmp/conversation_memory/agent_grounding.json` 保存 web、search、document、manual 或 tool evidence 的标题、查询、摘要、引用、置信度、tag 和状态。`GET/POST /api/agent/grounding` 可检索或登记 grounding source，`GET /api/agent/grounding/bundle` 会按查询、tag 和置信度生成带 citation index 的上下文包，`GET /api/agent/grounding/{source_id}` 可读取单条证据，`POST /api/agent/grounding/{source_id}/status` 可把证据标记为 active、stale 或 archived。它只记录可复用证据，不自动联网抓取页面。

Agent Fleet Control Plane 用 `.tmp/conversation_memory/agent_fleet.json` 保存本地多 Agent 实例目录、心跳、负载和任务分配。`GET/POST /api/agent/fleet` 可列出或登记实例，`POST /api/agent/fleet/{instance_id_or_name}/heartbeat` 可回写状态和指标，`POST /api/agent/fleet/{instance_id_or_name}/assignments` 可把 task/thread/workflow 分配给实例，`POST /api/agent/fleet/{instance_id_or_name}/assignments/{assignment_id}/status` 可更新分配状态，供未来 worker 或多实例 runner 接力。
